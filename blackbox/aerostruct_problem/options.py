...
from ..wing_problem.wing_vsp import WingVSP

class WingAeroStructOptions(BaseModel):
    """
        This class is used to define various settings for running
        aerostructural simulations for a wing geometry using ADflow
        and TACS. The wing shape is parameterized using OpenVSP.
    """
    ...
    struct_mesh_file: str = Field(
        description="Path to the wingbox structural mesh BDF file for the FE analysis"
    )
    ...
    wing_vsp: WingVSP = Field(
        exclude=True,
        default=None,
        description="Instance of `WingVSP` class defining various shape parameters. This argument is mandatory for setting shape parameters"
    )

    element_callback_function: Callable = Field(
        exclude=True,
        default=None,
        description="Callable that is used by TACS to setup elements for structural sizing"
    )
    ...
    write_structural_output: bool = Field(
        default=False,
        description="If True, write a HDF5 file containting all the structural outputs from TACS"
    )
    ...
    def post_init_validation(self):
        ...
        # set paths
        self.aero_mesh_file = os.path.abspath(self.aero_mesh_file)
        self.struct_mesh_file = os.path.abspath(self.struct_mesh_file)
        self.directory = os.path.abspath(self.directory)
        
        # check if path of mesh files exists
        if not os.path.exists(self.aero_mesh_file):
            raise ValueError("Provided aero cgns mesh file path does not exist")
        
        if not os.path.exists(self.struct_mesh_file):
            raise ValueError("Provided wingbox fem mesh file path does not exist")
