...
class WingAeroStructOptions(BaseProblem):
    ...
    def __init__(self, options: WingAeroStructOptions) -> None:
        ...
        assert isinstance(options, WingAeroStructOptions), "options argument should be an object of AirfoilOptions class"
        ...
        try:
            ...
            raise RuntimeError(
                "TACS is not installed or can not be imported\n\n"
                "You can follow the installation guide: https://smdogroup.github.io/tacs/install.html"
            )
        else:
            MIN_VERSION = parse_version("3.7.2")
            CURR_VERSION = parse_version(version("tacs"))
            assert CURR_VERSION >= MIN_VERSION, f"TACS version is {CURR_VERSION} but minimum {MIN_VERSION} is required"

        try:
            import funtofem
        except:
            raise RuntimeError(
                "FUNtoFEM is not installed or can not be imported\n\n"
                "You can follow the installation guide: https://smdogroup.github.io/funtofem/install.html"
            )
        else:
            MIN_VERSION = parse_version("0.3.7")
            CURR_VERSION = parse_version(version("funtofem"))
            assert CURR_VERSION >= MIN_VERSION, f"FUNtoFEM version is {CURR_VERSION} but minimum {MIN_VERSION} is required"

        try:
            import mphys
        except:
            raise RuntimeError(
                "MPhys is not installed or can not be imported\n\n"
                "You can follow the installation guide: https://github.com/OpenMDAO/mphys"
            )
        else:
            REQ_VERSION = parse_version("1.3.0")
            CURR_VERSION = parse_version(version("mphys"))
            assert CURR_VERSION == REQ_VERSION, f"MPhys version is {CURR_VERSION} but {REQ_VERSION} is required"

        try:
            import idwarp
        except:
            raise RuntimeError(
                "IDwarp is not installed or can not be imported\n\n"
                "You can follow the installation guide: https://mdolab-idwarp.readthedocs-hosted.com/en/latest/index.html"
            )
        else:
            MIN_VERSION = parse_version("2.6.2")
            CURR_VERSION = parse_version(version("idwarp"))
            assert CURR_VERSION >= MIN_VERSION, f"IDwarp version is {CURR_VERSION} but minimum {MIN_VERSION} is required"
    ...
    def __call__(self, x: np.ndarray, return_results: bool = False) -> None | dict:
        """
            ...
            size (d,) or (N,d) where N is the number of samples and d is the number of parameters added
            ...
            return_results: bool
                ...
                return the results. `NOTE`: This is only useful when you are doing sequential data generation
        """
    ...
    def _run_analysis(self, x: np.ndarray) -> None:
        ...
        filepath = os.path.join(pkgdir, "aerostruct_problem/runscript.py")
    ...
    def _create_input_file(self, x: np.ndarray) -> None:
        ...
        input = {
            ...
            "element_callback_function": self.options.element_callback_function,
            # write options
    ...
    def _write_initial_description(self, description) -> None:
        ...
        # Initial message
        description.write("--------------------------------------------------\n")
        description.write("Wing aerostructural analysis\n")
        description.write("--------------------------------------------------\n")
